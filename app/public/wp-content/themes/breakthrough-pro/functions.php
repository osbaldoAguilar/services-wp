...
add_filter( 'wp_resource_hints', 'breakthrough_resource_hints', 10, 2 );
/**
 ...
 */
function breakthrough_resource_hints( $urls, $relation_type ) {
...
add_filter( 'body_class', 'breakthrough_body_classes' );
/**
 ...
 */
function breakthrough_body_classes( $classes ) {
...
add_action( 'genesis_before', 'breakthrough_js_nojs_script', 1 );
/**
 * Echo out the script that changes 'no-js' class to 'js'.
 *
 * @since 1.3.1
 */
function breakthrough_js_nojs_script() {
...
add_action( 'after_setup_theme', 'breakthrough_widget_areas' );
/**
 * Registers Breakthrough widget areas.
 *
 * @since 1.3.1
 */
function breakthrough_widget_areas() {
...
function breakthrough_above_footer_cta() {
	...
	$footer_cta_image_id      = attachment_url_to_postid( $footer_cta_image );
	$footer_cta_image_default = breakthrough_get_default_page_footer_image();

	if ( $footer_cta_image ) {
		if ( $footer_cta_image_id ) {
			$image = wp_get_attachment_image( $footer_cta_image_id, 'header-image' );
		} else {
			$image = '<img width="1600" height="420" alt="" loading="lazy" src="' . $footer_cta_image_default . '" />';
		}

		echo '<div class="full-width-image footer-cta-image">' . $image . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
