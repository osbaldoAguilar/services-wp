...
function breakthrough_front_page_widgets() {
	...
	if ( is_active_sidebar( 'front-page-1' ) || is_active_sidebar( 'front-page-2' ) || is_active_sidebar( 'front-page-3' ) ) {
		...
		$front_page_image_2_id      = attachment_url_to_postid( $front_page_image_2 );
		$front_page_image_2_default = breakthrough_get_default_front_page_image_2();

		if ( $front_page_image_2 ) {
			if ( $front_page_image_2_id ) {
				$front_page_image_2_image = wp_get_attachment_image( $front_page_image_2_id, 'header-image' );
			} else {
				$front_page_image_2_image = '<img width="1600" height="420" alt="" loading="lazy" src="' . $front_page_image_2_default . '" />';
			}

			echo '<div class="full-width-image">' . $front_page_image_2_image . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	if ( is_active_sidebar( 'front-page-4' ) || is_active_sidebar( 'front-page-5' ) ) {
		...
		$front_page_image_3_id      = attachment_url_to_postid( $front_page_image_3 );
		$front_page_image_3_default = breakthrough_get_default_front_page_image_3();

		if ( $front_page_image_3 ) {
			if ( $front_page_image_3_id ) {
				$front_page_image_3_image = wp_get_attachment_image( $front_page_image_3_id, 'header-image' );
			} else {
				$front_page_image_3_image = '<img width="1600" height="420" alt="" loading="lazy" src="' . $front_page_image_3_default . '" />';
			}

			echo '<div class="full-width-image">' . $front_page_image_3_image . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
