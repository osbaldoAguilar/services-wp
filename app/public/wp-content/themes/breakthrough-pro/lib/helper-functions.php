...
function breakthrough_get_default_page_header_image() {
	...
	$header_image         = get_theme_mod( 'breakthrough_default_image', breakthrough_get_default_front_page_image_1() );
	$header_image_id      = attachment_url_to_postid( $header_image );
	$header_image_default = breakthrough_get_default_front_page_image_1();

	if ( $header_image ) {
		if ( $header_image_id ) {
			$header_image = genesis_get_image(
				[
					'format'   => 'html',
					'size'     => 'header-image',
					'context'  => '',
					'fallback' => $header_image_id,
					'attr'     => [
			...
			$header_image = '<img width="1600" height="420" alt="" loading="lazy" class="breakthrough-single-image post-image" src="' . $header_image_default . '" />';
		}
