...
class Subscription extends ComponentAbstract {
	 ...
	 * @var string
	 */
	const SUBSCRIPTION_KEY_SETTINGS_GROUP = 'genesis-custom-blocks-pro-subscription-key';
	...
	/**
	 * Gets the subscription key to save if it's valid, or false if it isn't.
	 *
	...
	public function save_subscription_key( $key ) {
		...
		return $subscription_response->is_valid() ? $sanitized_key : false;
	}
	 ...
	 */
	public function get_subscription_empty_notice() {
		return sprintf(
			'<div class="notice notice-error"><p>%1$s</p></div>',
			esc_html__( 'The subscription key was empty. Please enter a subscription key.', 'genesis-custom-blocks-pro' )
		);
	}
